...
- `Views/PlayerCellView.swift` — One player's life block. Composes a `DotNumberView` (life total) and a `PlayerCellBadgeBar` (commander damage + counters), rotated together to match cell orientation.
  ...
  - **Net-change readout.** Each life change (taps *and* ±10 hold ticks) accumulates into a running `sessionDelta`; a `deltaLabel` (Karl, `Typography.lifeDelta`) shows the **magnitude** next to the active side's ± icon — the icon *is* the sign, so the label is digits only (`5`, not `+5`). It's **net-signed**, so +7 then −2 reads as a "5" beside the now-bright plus icon. Positioning (in `registerDelta` → animated `layoutIfNeeded`, laid out in `layoutSubviews`): for a positive net the plus icon stays put and the digits slot in **outboard** (to its right); for a negative net the minus icon **slides left** (spring) to open room and the digits sit between it and the number, so it reads `−5`. The active side's icon animates to full opacity (1.0); the other stays at `adjustIconAlpha`. After `deltaIdleTimeout` (1.4s) of no input, `endDeltaSession` fades the label (frozen in place so it doesn't jump sides), glides the minus icon back to resting, and dims both icons; `sessionDelta` resets. Net zero ends the session early. `deltaSpacing` (8pt) is the tight sign-icon→digit gap. Torn down on edit/reset via `cancelDeltaSession` (called from `snapToOff` and the `isBeingEdited` setter); fades with the dots during swipe-out.
