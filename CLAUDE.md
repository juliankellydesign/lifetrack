...
- `Views/PlayerCellView.swift` — One player's life block. Composes a `DotNumberView` (life total) and a `PlayerCellBadgeBar` (commander damage + counters), rotated together to match cell orientation.
  ...
  - **Touch model** (zones are relative to the *player's* orientation, not the screen):
    - **Left / right thirds**: single tap commits ±1 on touchUp (light haptic). Holding past `holdActivationDelay` (0.5s) starts repeating ±10 every `repeatInterval` (0.35s) with a medium haptic per tick. If a hold starts repeating, the touchUp tap is suppressed. **Latency gotcha:** the tap commits on touchUp, and the board's swipe-to-reset `UIPanGestureRecognizer` would otherwise hold each `touchesEnded` back (`delaysTouchesEnded` defaults to true) until the pan resolves — visibly delaying every tap and bunching rapid ones. `GameBoardView.setupResetGesture` sets `delaysTouchesEnded = false` so touchUp (and the increment) lands immediately; a real swipe still cancels the cell touch via `cancelsTouchesInView`.
  ...
  - **Rapid-tap responsiveness (interrupt-driven, not time-thresholded).** Single taps pass `interruptible: true` down `applyChange` → `updateNumber` → `setDigit`; bulk ±10 repeats pass `false`. Each `DotDigitView` tracks `animatingUntil` — the wall-clock time (`CACurrentMediaTime`) at which its current roll finishes settling (`rollWindow` = 0.3s spring + the largest row delay ≈ 0.44s). `setDigit` then decides per change:
    - **Snap** (no spring) when the set isn't animated, **or** an interruptible change lands while a roll is still in flight (`now < animatingUntil`). It `removeAllAnimations()` and jumps every dot straight to the **new** digit, then *re-extends* `animatingUntil`, so the rest of a fast burst keeps snapping until you pause longer than one roll. Snapping to the *new* digit — not finalizing the previous one — is the fix: earlier versions either gated on a fixed tap-cadence threshold or finalized to the previous frame, and the visible number lagged a tap behind ("every other tap").
    - **Roll** (staggered spring, 0.3s / 0.7 damping) when nothing is in flight, or for any bulk repeat (which always rolls regardless of timing).
    Net: an isolated tap gets the full staggered roll; sustained fast tapping snaps each digit instantly and stays locked to the finger; ±10 holds keep the staggered look. The crossover is automatic — it's "did the previous roll finish?", so it adapts to any tap speed instead of a magic interval.
...
- **Animation.** Per-dot spring animations with row-based delay (`UIView.animate(withDuration: 0.3, delay:, usingSpringWithDamping: 0.7, ...)`). `ChangeDirection` is set in the same state transaction as the value change so dots see the correct delay. `setDigit(interruptible:)` snaps straight to the new digit (no spring) when an interruptible tap lands while the previous roll is still in flight, so fast tapping always shows the latest number — see the rapid-tap responsiveness note above. The single-`DotDigitView` state that drives this is `animatingUntil`. The post-reset roll-in reuses the same per-dot animation but pre-snaps every dot off via `snapToOff()` and adds a clockwise inter-cell stagger (see "Reset flow"). When you add a new animation that touches dots, prefer composing these primitives over writing a new one.
