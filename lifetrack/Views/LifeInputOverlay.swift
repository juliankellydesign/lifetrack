...
class LifeInputOverlay: UIView {
  ...
  /// laid-out region (counter row, life number, number pad) and each number-pad
  /// key. Mirrors `GameBoardView.showsGridSkeleton`.
  ...
  override func layoutSubviews() {
    ...
    // in the top-left track and the life number spans the remaining tracks.
    // Commander damage has its own board-wide mode. The dot-number hero
    ...
    if isHorizontal {
      ...
      let lifeAreaH = padCellH * 3 + padRowSpacing * 2
  ...
  private func updateSkeleton() {
    ...
    for region in [counterRow.frame, dotNumberView.frame, numberPadView.frame] {
